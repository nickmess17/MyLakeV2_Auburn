...
lake='Lake Auburn';
year=2009;
m_start=[2009,4,14]; %year,month,day
m_stop=[2017,11,14];
...
%physical parameters
lake_params{2} = 0.017; % 0.0442401  % 2     open water diffusion parameter (-)
lake_params{5} = 0.68; % 0.5 % 5     wind shelter parameter (-)
lake_params{16} = 1.946; % 1 % 16    scaling factor for inflow volume (-)
lake_params{17} = 0; % 0 % 17    adjusting delta for inflow temperature (-)
lake_params{39} = 0.2109; % 2.5 % 39     non-PAR light attenuation coeff. (m-1)
lake_params{40} = 1.4937; %1.05 %40    PAR light attenuation coefficient for water (m-1)
%biological parameters
lake_params{10} = 0.00005; % 10    PAR saturation level for phytoplankton growth (mol(quanta) m-2 s-1)
lake_params{20} = 1.5; % 20    scaling factor for inflow concentration of total P (-)
lake_params{56} = 0.1; % 56    Settling velocity for Chl a (m day-1)
lake_params{57} = 0.1; % 57    Loss rate (1/day) at 20 deg C
lake_params{58} = 1; % 58    Specific growth rate (1/day) at 20 deg C
lake_params{53} = 0.5;% 53    Half saturation growth P level (mg/m3)
...
% grid on;
% 
 tlims=[datenum([2013,1,1]) datenum([2018,1,1])];
% 
...
% hold on
% plot(tt_mod+datenum(year,1,1),MyLake_results.basin1.T(1,:),'-');
%  set(gca,'ylim',[0 30]);
...
% hold on
% plot(tt_mod+datenum(year,1,1),MyLake_results.basin1.T(6,:),'-');
%  set(gca,'ylim',[0 30]);
%  ylabel('^oC','fontsize',9)
...
% plot(TempObs(inx,1)+datenum(year,1,1),TempObs(inx,3),'r.','linewidth',2);
% hold on
% plot(tt_mod+datenum(year,1,1),MyLake_results.basin1.T(10,:),'-');
%  set(gca,'ylim',[0 30]);
%  ylabel('^oC','fontsize',9)
...
% plot(TempObs(inx,1)+datenum(year,1,1),TempObs(inx,3),'r.','linewidth',2);
% hold on
% plot(tt_mod+datenum(year,1,1),MyLake_results.basin1.T(16,:),'-');
%  set(gca,'ylim',[0 25]);
%  ylabel('^oC','fontsize',9)
...
% plot(TempObs(inx,1)+datenum(year,1,1),TempObs(inx,3),'r.','linewidth',2);
% hold on
% plot(tt_mod+datenum(year,1,1),MyLake_results.basin1.T(34,:),'-');
%  set(gca,'ylim',[0 25]);
%  ylabel('^oC','fontsize',9)
...
clf
subplot(211)
inx=find(round(DOobs(:,2))==0);
I=plot(DOobs(inx,1)+datenum(year,1,1),DOobs(inx,3),'r.','linewidth',2);
hold on
plot(tt_mod+datenum(year,1,1),MyLake_results.basin1.concentrations.O2(1,:),'b-');
 set(gca,'ylim',[0 20]);
 ylabel('mg/L','fontsize',9)
 title('DO  0m','fontweight','bold')
 datetick('x','yy')
 grid on
 set(gca,'fontsize',9)
 set(gca,'xticklabel',[]);
...
hold on
plot(tt_mod+datenum(year,1,1),MyLake_results.basin1.concentrations.O2(34,:),'b-');
 set(gca,'ylim',[0 20]);
 ylabel('mg/L','fontsize',9)
 xlabel('year','fontsize',9)
 title('DO 33m','fontweight','bold')
 datetick('x','yy')
 grid on
 set(gca,'fontsize',9)
 set(gca,'xlim',tlims)